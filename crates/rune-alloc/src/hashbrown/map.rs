...
use crate::alloc::{into_ok, into_ok_try};
...
///
/// vikings.try_insert(Viking::new("Einar", "Norway"), 25)?;
/// vikings.try_insert(Viking::new("Olaf", "Denmark"), 24)?;
/// vikings.try_insert(Viking::new("Harald", "Iceland"), 12)?;
///
...
/// instances of any functions like RawTable::reserve from being generated
#[cfg_attr(feature = "inline-more", inline)]
fn equivalent_key<C, Q, K, V>(k: &Q) -> impl Fn(&mut C, &(K, V)) -> Result<bool, Infallible> + '_
where
    Q: ?Sized + Equivalent<K>,
{
    move |_, x| Ok(k.equivalent(&x.0))
}
...
impl<K, V> HashMap<K, V, DefaultHashBuilder> {
    ...
    /// allow the `HashMap` to be protected against attacks such as [`HashDoS`].
    /// Users who require HashDoS resistance should explicitly use
    /// [`ahash::RandomState`] or [`std::collections::hash_map::RandomState`] as
    /// the hasher when creating a [`HashMap`], for example with
    ...
    ///
    /// [`try_with_capacity_and_hasher`]: HashMap::try_with_capacity_and_hasher
    /// [`HashDoS`]: https://en.wikipedia.org/wiki/Collision_attack
    /// [`std::collections::hash_map::RandomState`]: https://doc.rust-lang.org/std/collections/hash_map/struct.RandomState.html
    ///
    /// # Examples
    ...
    /// ```
    /// use rune::alloc::HashMap;
    /// let mut map: HashMap<&str, i32> = HashMap::try_with_capacity(10)?;
    /// assert_eq!(map.len(), 0);
...
impl<K, V, A: Allocator> HashMap<K, V, DefaultHashBuilder, A> {
    ...
    ///     HashMap::try_with_capacity_and_hasher_in
    /// [`HashDoS`]: https://en.wikipedia.org/wiki/Collision_attack
    /// [`std::collections::hash_map::RandomState`]: https://doc.rust-lang.org/std/collections/hash_map/struct.RandomState.html
    ///
    /// # Examples
    ...
    /// use rune::alloc::alloc::Global;
    ///
    /// let mut map = HashMap::try_with_capacity_in(5, Global)?;
    ///
    /// // The created HashMap holds none elements
    /// assert_eq!(map.len(), 0);
    ...
    /// map.try_insert("One",   1)?;
    /// map.try_insert("Two",   2)?;
    /// map.try_insert("Three", 3)?;
    /// map.try_insert("Four",  4)?;
    /// map.try_insert("Five",  5)?;
    ///
    ...
    pub fn try_with_capacity_in(capacity: usize, alloc: A) -> Result<Self, Error> {
        Self::try_with_capacity_and_hasher_in(capacity, DefaultHashBuilder::default(), alloc)
    }
}
...
impl<K, V, S> HashMap<K, V, S> {
    ...
    ///
    /// let s = DefaultHashBuilder::default();
    /// let mut map = HashMap::try_with_capacity_and_hasher(10, s)?;
    /// assert_eq!(map.len(), 0);
    /// assert!(map.capacity() >= 10);
    ///
    /// map.try_insert(1, 2)?;
    ...
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn try_with_capacity_and_hasher(capacity: usize, hash_builder: S) -> Result<Self, Error> {
        Ok(Self {
            hash_builder,
            table: RawTable::try_with_capacity_in(capacity, Global)?,
        })
    }

    #[cfg(test)]
    pub(crate) fn with_capacity_and_hasher(capacity: usize, hash_builder: S) -> Self {
        Self::try_with_capacity_and_hasher(capacity, hash_builder).abort()
    }
}
...
impl<K, V, S, A: Allocator> HashMap<K, V, S, A> {
    ...
    ///
    /// let s = DefaultHashBuilder::default();
    /// let mut map = HashMap::try_with_capacity_and_hasher_in(10, s, Global)?;
    /// map.try_insert(1, 2)?;
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    ...
    /// ```
    /// use rune::alloc::HashMap;
    /// let map: HashMap<i32, i32> = HashMap::try_with_capacity(100)?;
    /// assert_eq!(map.len(), 0);
    ...
    ///
    /// ```
    /// use rune::alloc::{HashMap, Vec};
    /// use rune::alloc::prelude::*;
    ///
    /// let mut map: HashMap<i32, i32> = (0..8).map(|x|(x, x*10)).try_collect()?;
    /// assert_eq!(map.len(), 8);
    ///
    ...
    /// assert_eq!(map.len(), 4);
    ///
    /// let mut vec: Vec<(i32, i32)> = map.iter().map(|(&k, &v)| (k, v)).try_collect()?;
    /// vec.sort_unstable();
    ...
    ///
    /// ```
    /// use rune::alloc::{try_vec, HashMap, Vec};
    /// use rune::alloc::prelude::*;
    ///
    /// let mut map: HashMap<i32, i32> = (0..8).map(|x| (x, x)).try_collect()?;
    ///
    /// let drained: HashMap<i32, i32> = map.extract_if(|k, _v| k % 2 == 0).try_collect()?;
    ///
    /// let mut evens = drained.keys().cloned().try_collect::<Vec<_>>()?;
    /// let mut odds = map.keys().cloned().try_collect::<Vec<_>>()?;
    /// evens.sort();
    /// odds.sort();
    ///
    ...
    /// map.try_insert("c", 3)?;
    ///
    /// let mut vec: Vec<&str> = map.into_keys().try_collect()?;
    ///
    ...
    /// map.try_insert("c", 3)?;
    ///
    /// let mut vec: Vec<i32> = map.into_values().try_collect()?;
    ///
    ...
    A: Allocator,
{
    ...
    pub fn try_reserve(&mut self, additional: usize) -> Result<(), Error> {
        let hasher = make_hasher::<K, S>(&self.hash_builder);
        into_ok_try(
            self.table
                .try_reserve(&mut (), additional, hasher.into_tuple()),
        )
    }
    ...
    /// use rune::alloc::HashMap;
    ///
    /// let mut map: HashMap<i32, i32> = HashMap::try_with_capacity(100)?;
    /// map.try_insert(1, 2)?;
    /// map.try_insert(3, 4)?;
    /// assert!(map.capacity() >= 100);
    /// map.try_shrink_to_fit()?;
    /// assert!(map.capacity() >= 2);
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn try_shrink_to_fit(&mut self) -> Result<(), Error> {
        into_ok_try(self.table.shrink_to(
            &mut (),
            0,
            make_hasher::<K, S>(&self.hash_builder).into_tuple(),
        ))
    }
    ...
    ///     let counter = letters.entry(ch).or_try_insert(0)?;
    ///     *counter += 1;
    /// }
    ///
    ...
    pub fn entry(&mut self, key: K) -> Entry<'_, K, V, S, A> {
        ...
        if let Some(elem) = into_ok(self.table.find(&mut (), hash, equivalent_key(&key))) {
    ...
    ///     let counter = words.entry_ref(s).or_try_insert(0)?;
    ///     *counter += 1;
    /// }
    ///
        ...
        Q: ?Sized + Hash + Equivalent<K>,
    {
        if self.table.is_empty() {
            ...
            into_ok(self.table.get(&mut (), hash, equivalent_key(k)))
        }
    }
        ...
        Q: ?Sized + Hash + Equivalent<K>,
    {
        if self.table.is_empty() {
            ...
            let hash = make_hash::<Q, S>(&self.hash_builder, k);
            into_ok(self.table.get_mut(&mut (), hash, equivalent_key(k)))
        }
    }
    ...
    /// libraries.try_insert("Bodleian Library".to_string(), 1602)?;
    /// libraries.try_insert("Athenæum".to_string(), 1807)?;
    /// libraries.try_insert("Herzogin-Anna-Amalia-Bibliothek".to_string(), 1691)?;
    /// libraries.try_insert("Library of Congress".to_string(), 1800)?;
    ///
        ...
        Q: ?Sized + Hash + Equivalent<K>,
    {
        ...
        into_ok(
            self.table
                .get_many_mut(&mut (), hashes, |_, i, (k, _)| Ok(ks[i].equivalent(k))),
        )
    }
        ...
        Q: ?Sized + Hash + Equivalent<K>,
    {
        ...
        into_ok(
            self.table
                .get_many_unchecked_mut(&mut (), hashes, |_, i, (k, _)| Ok(ks[i].equivalent(k))),
        )
    }
    ...
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn try_insert(&mut self, k: K, v: V) -> Result<Option<V>, Error> {
        let hasher = make_hasher::<K, S>(&self.hash_builder);
        let hash = into_ok(hasher.hash(&mut (), &k));
    ...
    /// assert_eq!(map1.try_insert(1, "a")?, None);
    /// assert_eq!(map1.try_insert(2, "b")?, None);
    /// assert_eq!(map1.try_insert(3, "c")?, None);
    /// assert_eq!(map1.len(), 3);
    ///
    /// let mut map2 = HashMap::new();
    ///
    /// for (key, value) in map1.into_iter() {
    ///     map2.try_insert_unique_unchecked(key, value)?;
    /// }
    ///
    /// let (key, value) = map2.try_insert_unique_unchecked(4, "d")?;
    /// assert_eq!(key, &4);
    ...
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn try_insert_unique_unchecked(&mut self, k: K, v: V) -> Result<(&K, &mut V), Error> {
        let hasher = make_hasher::<K, S>(&self.hash_builder);
        let hash = into_ok(hasher.hash(&mut (), &k));
        let bucket = into_ok_try(
            self.table
                .insert(&mut (), hash, (k, v), hasher.into_tuple()),
        )?;
    ...
    #[cfg(test)]
    pub(crate) fn insert_unique_unchecked(&mut self, k: K, v: V) -> (&K, &mut V) {
    ...
    /// use rune::alloc::hash_map::OccupiedError;
    ///
    /// let mut map = HashMap::new();
    /// assert_eq!(map.try_insert_or(37, "a").unwrap(), &"a");
    ///
    /// match map.try_insert_or(37, "b") {
    ///     Err(CustomError::Custom(OccupiedError { entry, value })) => {
    ...
    pub fn try_insert_or(
        ...
        key: K,
        value: V,
    ) -> Result<&mut V, CustomError<OccupiedError<'_, K, V, S, A>>> {
        match self.entry(key) {
            Entry::Occupied(entry) => Err(CustomError::Custom(OccupiedError { entry, value })),
            Entry::Vacant(entry) => Ok(entry.try_insert(value)?),
        }
    }
...
impl<K, V, S, A: Allocator> HashMap<K, V, S, A> {
    ...
    ///         let (k, value) = view.try_insert("d", 4000)?;
    ...
    ///     match raw_table.find(&mut (), hash, |_: &mut (), k: &(K, V)| Ok::<_, Infallible>(is_match(k))).unwrap() {
    ...
    A: Allocator,
{
    ...
    /// use rune::alloc::HashMap;
    ///
    /// let map: HashMap<_, _> = [("a", "One"), ("b", "Two")].try_into()?;
    ///
    /// assert_eq!(map[&"a"], "One");
    /// assert_eq!(map[&"b"], "Two");
    /// # Ok::<_, rune::alloc::Error>(())
...
impl<K, V, A, const N: usize> TryFrom<[(K, V); N]> for HashMap<K, V, DefaultHashBuilder, A>
where
    K: Eq + Hash,
    A: Default + Allocator,
{
    ...
    /// use rune::alloc::HashMap;
    ///
    /// let map1 = HashMap::try_from([(1, 2), (3, 4)])?;
    /// let map2: HashMap<_, _> = [(1, 2), (3, 4)].try_into()?;
    /// assert_eq!(map1, map2);
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    fn try_from(arr: [(K, V); N]) -> Result<Self, Self::Error> {
        HashMap::try_from_iter_in(arr, A::default())
    }
}
...
/// ```
/// use core::hash::{BuildHasher, Hash};
/// use rune::alloc::hash_map::{RawEntryMut, RawVacantEntryMut};
/// use rune::alloc::HashMap;
///
...
///     RawEntryMut::Occupied(_) => unreachable!(),
///     RawEntryMut::Vacant(view) => {
///         let (k, value) = view.try_insert("b", 2)?;
...
///     RawEntryMut::Occupied(_) => unreachable!(),
///     RawEntryMut::Vacant(view) => {
///         assert_eq!(view.try_insert("c", 30)?, (&mut "c", &mut 30));
///     }
/// }
...
/// ```
/// use core::hash::{BuildHasher, Hash};
/// use rune::alloc::hash_map::RawEntryBuilder;
/// use rune::alloc::HashMap;
/// use rune::alloc::prelude::*;
///
/// let mut map = HashMap::new();
...
impl<'a, K, V, S, A: Allocator> RawEntryBuilderMut<'a, K, V, S, A> {
    ...
    where
        F: FnMut(&K) -> bool,
    {
        match into_ok(self.map.table.find(
            &mut is_match,
            hash,
            move |is_match: &mut F, (k, _): &(K, _)| Ok(is_match(k)),
        )) {
...
impl<'a, K, V, S, A: Allocator> RawEntryBuilder<'a, K, V, S, A> {
    ...
    /// use rune::alloc::HashMap;
    ///
    /// let map: HashMap<&str, u32> = [("a", 100), ("b", 200)].try_into()?;
    /// let key = "a";
    ...
    where
        F: FnMut(&K) -> bool,
    {
        match into_ok(self.map.table.get(
            &mut is_match,
            hash,
            |is_match: &mut F, (k, _): &(K, _)| Ok(is_match(k)),
        )) {
            Some((key, value)) => Some((key, value)),
            None => None,
        }
...
impl<'a, K, V, S, A: Allocator> RawEntryMut<'a, K, V, S, A> {
    ...
    ///
    /// let mut map: HashMap<&str, u32> = HashMap::new();
    /// let entry = map.raw_entry_mut().from_key("horseyland").try_insert("horseyland", 37)?;
    ///
    /// assert_eq!(entry.remove_entry(), ("horseyland", 37));
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn try_insert(self, key: K, value: V) -> Result<RawOccupiedEntryMut<'a, K, V, S, A>, Error>
    where
        K: Hash,
        S: BuildHasher,
    {
        match self {
            ...
            RawEntryMut::Vacant(entry) => {
                let hasher = make_hasher::<K, S>(entry.hash_builder);
                into_ok_try(entry.insert_entry(&mut (), hasher, key, value))
            }
        }
    }

    #[cfg(test)]
    pub(crate) fn insert(self, key: K, value: V) -> RawOccupiedEntryMut<'a, K, V, S, A>
    where
        K: Hash,
        S: BuildHasher,
    {
    ...
    /// let mut map: HashMap<&str, u32> = HashMap::new();
    ///
    /// map.raw_entry_mut().from_key("poneyland").or_try_insert("poneyland", 3)?;
    /// assert_eq!(map["poneyland"], 3);
    ///
    /// *map.raw_entry_mut().from_key("poneyland").or_try_insert("poneyland", 10)?.1 *= 2;
        ...
        S: BuildHasher,
    {
        match self {
            ...
            RawEntryMut::Vacant(entry) => entry.try_insert(default_key, default_val),
        }
    }
    ...
    ///
    /// map.raw_entry_mut().from_key("poneyland").or_try_insert_with(|| {
    ...
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn or_try_insert_with<F>(self, default: F) -> Result<(&'a mut K, &'a mut V), Error>
    where
        F: FnOnce() -> (K, V),
        K: Hash,
        S: BuildHasher,
...
impl<'a, K, V, S, A: Allocator> RawOccupiedEntryMut<'a, K, V, S, A> {
    ...
    /// use rune::alloc::hash_map::{HashMap, RawEntryMut};
    ///
    /// let mut map: HashMap<&str, u32> = [("a", 100), ("b", 200)].try_into()?;
    ///
    /// match map.raw_entry_mut().from_key(&"a") {
    ///     RawEntryMut::Vacant(_) => panic!(),
    ...
    /// map.try_insert(key_one.clone(), 10)?;
    ///
...
impl<'a, K, V, S, A: Allocator> RawVacantEntryMut<'a, K, V, S, A> {
    ...
    ///     RawEntryMut::Occupied(_) => panic!(),
    ///     RawEntryMut::Vacant(v) => assert_eq!(v.try_insert("c", 300)?, (&mut "c", &mut 300)),
    /// }
    ///
    /// assert_eq!(map[&"c"], 300);
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn try_insert(self, key: K, value: V) -> Result<(&'a mut K, &'a mut V), Error>
    where
        K: Hash,
        S: BuildHasher,
    {
        let hasher = make_hasher(self.hash_builder);
        let hash = into_ok(hasher.hash(&mut (), &key));

        let &mut (ref mut k, ref mut v) =
            into_ok_try(
                self.table
                    .insert_entry(&mut (), hash, (key, value), hasher.into_tuple()),
            )?;
    ...
    #[cfg(test)]
    pub(crate) fn insert(self, key: K, value: V) -> (&'a mut K, &'a mut V)
    where
        K: Hash,
        S: BuildHasher,
    {
    ...
    ///         v.try_insert_hashed_nocheck(hash, key, 300)?,
    ///         (&mut "c", &mut 300)
    ///     ),
    /// }
    ///
    ...
    ///     RawEntryMut::Occupied(_) => panic!(),
    ///     RawEntryMut::Vacant(v) => assert_eq!(
    ///         v.try_insert_with_hasher(hash, key, 100, make_hasher(&hash_builder))?,
    ///         (&mut "a", &mut 100)
    ///     ),
    /// }
    ///
    /// map.try_extend([("b", 200), ("c", 300), ("d", 400), ("e", 500), ("f", 600)])?;
    /// assert_eq!(map[&"a"], 100);
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    ...
    where
        H: Fn(&K) -> u64,
    {
        let &mut (ref mut k, ref mut v) = into_ok_try(self.table.insert_entry(
            &mut (),
            hash,
            (key, value),
            move |_: &mut (), x: &(K, V)| Ok(hasher(&x.0)),
        ))?;
    ...
    #[cfg(test)]
    pub(crate) fn insert_with_hasher<H>(
        self,
        hash: u64,
        key: K,
        value: V,
    ...
    where
        H: Fn(&K) -> u64,
    {
        self.try_insert_with_hasher(hash, key, value, hasher)
            .abort()
    }
    ...
    fn insert_entry<C, E>(
        self,
        cx: &mut C,
        hasher: impl HasherFn<C, K, E>,
        key: K,
        value: V,
    ) -> Result<RawOccupiedEntryMut<'a, K, V, S, A>, CustomError<E>>
    where
        K: Hash,
        S: BuildHasher,
    {
        let hash = hasher.hash(cx, &key).map_err(CustomError::Custom)?;

        let elem = self
            .table
            .insert(cx, hash, (key, value), hasher.into_tuple())?;
...
///
/// ```
/// use rune::alloc::hash_map::{HashMap, OccupiedError};
/// use rune::alloc::error::CustomError;
///
/// let mut map: HashMap<_, _> = [("a", 10), ("b", 20)].try_into()?;
///
...
/// match map.try_insert_or("a", 100) {
///     Err(CustomError::Custom(OccupiedError { mut entry, value })) => {
...
impl<'a, K, V, S, A: Allocator> IntoIterator for &'a HashMap<K, V, S, A> {
    ...
    /// ```
    /// use rune::alloc::HashMap;
    /// let map_one: HashMap<_, _> = [(1, "a"), (2, "b"), (3, "c")].try_into()?;
    /// let mut map_two = HashMap::new();
    ///
    /// for (key, value) in &map_one {
    ///     println!("Key: {}, Value: {}", key, value);
    ///     map_two.try_insert_unique_unchecked(*key, *value)?;
    /// }
    ///
...
impl<'a, K, V, S, A: Allocator> IntoIterator for &'a mut HashMap<K, V, S, A> {
    ...
    /// ```
    /// use rune::alloc::HashMap;
    /// let mut map: HashMap<_, _> = [("a", 1), ("b", 2), ("c", 3)].try_into()?;
    ///
...
impl<K, V, S, A: Allocator> IntoIterator for HashMap<K, V, S, A> {
    ...
    /// use rune::alloc::HashMap;
    ///
    /// let map: HashMap<_, _> = [("a", 1), ("b", 2), ("c", 3)].try_into()?;
    ///
...
impl<'a, K, V, S, A: Allocator> Entry<'a, K, V, S, A> {
        ...
        S: BuildHasher,
    {
        match self {
            ...
            Entry::Vacant(entry) => entry.try_insert_entry(value),
        }
    }

    #[cfg(test)]
    pub(crate) fn insert(self, value: V) -> OccupiedEntry<'a, K, V, S, A>
    where
        K: Hash,
        S: BuildHasher,
    {
...
impl<'a, K, V: Default, S, A: Allocator> Entry<'a, K, V, S, A> {
        ...
        S: BuildHasher,
    {
        match self {
            Entry::Occupied(entry) => Ok(entry.into_mut()),
            Entry::Vacant(entry) => entry.try_insert(Default::default()),
        }
    }
}
...
impl<'a, K, V, S, A: Allocator> OccupiedEntry<'a, K, V, S, A> {
    ...
    /// # Panics
    ///
    /// Will panic if this OccupiedEntry was created through [`Entry::try_insert`].
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// map.try_insert(key_one.clone(), 15)?;
    /// assert!(Rc::strong_count(&key_one) == 2 && Rc::strong_count(&key_two) == 1);
    ///
    ...
    /// use std::rc::Rc;
    ///
    /// let mut map: HashMap<Rc<String>, usize> = HashMap::try_with_capacity(6)?;
    ...
    ///     map.try_insert(rc_key.clone(), value)?;
...
impl<'a, K, V, S, A: Allocator> VacantEntry<'a, K, V, S, A> {
    ...
    #[cfg_attr(feature = "inline-more", inline)]
    pub(crate) fn try_insert_entry(self, value: V) -> Result<OccupiedEntry<'a, K, V, S, A>, Error>
    where
        K: Hash,
        S: BuildHasher,
    {
...
impl<'a, 'b, K, Q: ?Sized, V, S, A: Allocator> EntryRef<'a, 'b, K, Q, V, S, A> {
    ...
    /// let entry = map.entry_ref("horseyland").try_insert(37)?;
    ///
    /// assert_eq!(entry.key(), "horseyland");
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
        ...
        S: BuildHasher,
    {
        match self {
            ...
            EntryRef::Vacant(entry) => entry.try_insert_entry(value),
        }
    }
    ...
    ///
    /// // nonexistent key
    /// map.entry_ref("poneyland").or_try_insert(3)?;
    /// assert_eq!(map["poneyland"], 3);
    ///
    /// // existing key
    /// *map.entry_ref("poneyland").or_try_insert(10)? *= 2;
    /// assert_eq!(map["poneyland"], 6);
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
        ...
        S: BuildHasher,
    {
        match self {
            ...
            EntryRef::Vacant(entry) => entry.try_insert(default),
        }
    }
    ...
    ///
    /// // nonexistent key
    /// map.entry_ref("poneyland").or_try_insert_with(|| 3)?;
    /// assert_eq!(map["poneyland"], 3);
    ///
    /// // existing key
    /// *map.entry_ref("poneyland").or_try_insert_with(|| 10)? *= 2;
    /// assert_eq!(map["poneyland"], 6);
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    ...
    ///
    /// // nonexistent key
    /// map.entry_ref("poneyland").or_try_insert_with_key(|key| key.chars().count())?;
    /// assert_eq!(map["poneyland"], 9);
    ///
    /// // existing key
    /// *map.entry_ref("poneyland").or_try_insert_with_key(|key| key.chars().count() * 10)? *= 2;
    /// assert_eq!(map["poneyland"], 18);
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    ...
    /// ```
    /// use rune::alloc::HashMap;
    /// use rune::alloc::hash_map::EntryRef;
    ///
    /// let mut map: HashMap<String, u32> = HashMap::new();
    ///
    /// let entry = map
    ...
    /// }
    ///
    /// map.try_insert("poneyland".to_string(), 42)?;
    ///
    /// let entry = map
    ///     .entry_ref("poneyland")
    ///     .and_replace_entry_with(|k, v| {
...
impl<'a, 'b, K, Q: ?Sized, V: Default, S, A: Allocator> EntryRef<'a, 'b, K, Q, V, S, A> {
    ...
    ///
    /// // nonexistent key
    /// map.entry_ref("poneyland").or_try_default()?;
    /// assert_eq!(map["poneyland"], None);
    ///
    /// map.try_insert("horseland".to_string(), Some(3))?;
    ///
    /// // existing key
    /// assert_eq!(map.entry_ref("horseland").or_try_default()?, &mut Some(3));
    /// # Ok::<_, rune::alloc::Error>(())
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    pub fn or_try_default(self) -> Result<&'a mut V, Error>
        ...
        S: BuildHasher,
    {
        match self {
            EntryRef::Occupied(entry) => Ok(entry.into_mut()),
            EntryRef::Vacant(entry) => entry.try_insert(Default::default()),
        }
    }
}
...
impl<'a, 'b, K, Q: ?Sized, V, S, A: Allocator> OccupiedEntryRef<'a, 'b, K, Q, V, S, A> {
    ...
    ///
    /// let mut map: HashMap<String, u32> = HashMap::new();
    /// map.entry_ref("poneyland").or_try_insert(12)?;
    ///
    ...
    /// # Panics
    ///
    /// Will panic if this OccupiedEntryRef was created through [`EntryRef::try_insert`].
    ///
    /// # Examples
    ///
    /// ```
    ...
    S: BuildHasher + Default,
{
    ...
    fn try_from_iter_in<T: IntoIterator<Item = (K, V)>>(iter: T, alloc: A) -> Result<Self, Error> {
        ...
        let mut map =
            Self::try_with_capacity_and_hasher_in(iter.size_hint().0, S::default(), alloc)?;
    ...
    A: Allocator,
{
    ...
    ///
    /// let some_arr = [(5, 5), (6, 6)];
    /// map.try_extend(some_arr.iter().map(|(k, v)| (k, v)))?;
    ///
    /// // You can also extend from another HashMap
    /// let mut new_map = HashMap::new();
    /// new_map.try_extend(&map)?;
    ...
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    fn try_extend<T: IntoIterator<Item = (&'a K, &'a V)>>(&mut self, iter: T) -> Result<(), Error> {
    ...
    A: Allocator,
{
    ...
    /// ```
    #[cfg_attr(feature = "inline-more", inline)]
    fn try_extend<T: IntoIterator<Item = &'a (K, V)>>(&mut self, iter: T) -> Result<(), Error> {
        self.try_extend(iter.into_iter().map(|&(key, value)| (key, value)))
    }
}
...
mod test_map {
    ...
    use super::{EntryRef, HashMap, RawEntryMut};
    ...
    impl TryClone for Droppable {
        fn try_clone(&self) -> Result<Self, Error> {
            Ok(Droppable::new(self.k))
        }
    }
    ...
    fn test_extend_ref_kv_tuple() {
        ...
        a.try_extend(iter).abort();
        a.try_extend(&vec).abort();
        a.try_extend(create_arr::<i32, 100>(200, 1)).abort();
        ...
        for n in 0..N {
            ...
            loop {
                ...
                if i < n && rng.gen_bool(0.1) {
                    ...
                    unsafe {
                        let e = into_ok(map.table.find(
                            &mut (),
                            hash_value,
                            |_: &mut (), q: &(usize, _)| Ok(q.0.eq(&i)),
                        ));
                        if let Some(e) = e {
                            it.reflect_remove(&e);
                            ...
                            let e = into_ok_try(map.table.insert(
                                &mut (),
                                hash_value,
                                (i, 2 * i),
                                super::make_hasher(&hash_builder),
                            ))
                            .unwrap();
                            it.reflect_insert(&e);
    ...
    fn test_const_with_hasher() {
        ...
        const EMPTY_MAP: HashMap<u32, String, MyHasher> = HashMap::with_hasher(MyHasher);

        let mut map = EMPTY_MAP;
        map.try_insert(17, "seventeen".to_owned()).unwrap();
    ...
    fn test_clone_from_double_drop() {
        ...
        impl TryClone for CheckedDrop {
            fn try_clone(&self) -> Result<Self, crate::error::Error> {
                Ok(Self {
        ...
        let mut map1 = HashMap::new();
        map1.try_insert(1, DISARMED).unwrap();
        map1.try_insert(2, DISARMED).unwrap();
        map1.try_insert(3, DISARMED).unwrap();
        map1.try_insert(4, DISARMED).unwrap();

        let mut map2 = HashMap::new();
        map2.try_insert(1, DISARMED).unwrap();
        map2.try_insert(2, ARMED).unwrap();
        map2.try_insert(3, DISARMED).unwrap();
        map2.try_insert(4, DISARMED).unwrap();

        map2.try_clone_from(&map1).unwrap();
    }
    ...
    fn test_clone_from_memory_leaks() {
        ...
        impl TryClone for CheckedClone {
            fn try_clone(&self) -> Result<Self, Error> {
        ...
        .unwrap();
        let _map2 = map1.try_clone().unwrap();
    }
    ...
    fn test_hashmap_into_iter_bug() {
        ...
        {
            let mut map = HashMap::try_with_capacity_in(10, MyAlloc::new(dropped.clone())).unwrap();
            for i in 0..10 {
                map.entry(i).or_try_insert_with(|| "i".to_string()).unwrap();
            }
    ...
    impl<T> TryClone for CheckedCloneDrop<T>
    where
        T: TryClone,
    {
        fn try_clone(&self) -> Result<Self, Error> {
        ...
        T: PartialEq + core::fmt::Debug,
    {
        ...
        let mut map: HashMap<u64, CheckedCloneDrop<T>, _, A> =
            HashMap::try_with_capacity_in(iter.size_hint().0, alloc).unwrap();
        {
            ...
            for (panic_in_clone, panic_in_drop) in iter.clone() {
                ...
                into_ok_try(guard.table.insert(
                    &mut (),
                    count,
                    (
                        count,
    ...
    fn test_clone_memory_leaks_and_double_drop_one() {
...

        {
            ...
            let _map2 = map.try_clone().unwrap();
        }
    }
    ...
    fn test_clone_memory_leaks_and_double_drop_two() {
...

        {
            ...
            map2.try_clone_from(&map).unwrap();
        }
    }
...

        {
            ...
            thread::scope(|s| {
                let result: thread::ScopedJoinHandle<'_, String> = s.spawn(|| {
                    ...
                    map.try_clone_from(&scope_map).unwrap();
            ...
            for idx in 0..map.table.buckets() {
                ...
                assert!(
                    into_ok(
                        map.table
                            .find(&mut (), idx, |_: &mut (), (k, _): &(u64, _)| Ok(*k == idx))
                    )
                    .is_none(),
                    "Index: {idx}"
                );
